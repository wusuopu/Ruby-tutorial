...
根据Ruby的官方文档，*Object*是Ruby的类层级的根。它的所有方法在所有的类中都是一样的，除非显示的覆盖了。
...
以上这些代码全都返回*true*。数字6是*Fixnum*类型。从Ruby文档中我们发现另外四个对象是*Fuxnum*对象的父类。
...
在这个例子中我们创建了两个对象。*Being*和*Living*。*Living*对象继承至*Being*。第一个是父对象，第二介是子对象。
...
## Ruby的顶级环境
Ruby中有一个特殊的对象引用到Ruby的顶级环境，这是定义在其他上下文之外的默认执行环境。顶级环境名字是*main*。它是一个*Object*类型的实例对象。以下*main*分配了一个局部空间，所有的局部变量有属于它。
